namespace CoffeePOS.Shared.Constants;

public readonly struct SvgAssets
{
    public const string Cart = "ShoppingCartOutlined";
    public const string Dashboard = "DashboardOutlined";
    public const string FileText = "FileTextOutlined";
    public const string Box = "AppstoreOutlined";
    public const string Tags = "TagsOutlined";
    public const string Users = "TeamOutlined";
    public const string User = "UserOutlined";
    public const string Setting = "SettingOutlined";
    public const string Logout = "LogoutOutlined";
    public const string MenuFoldOutlined = "MenuFoldOutlined";
    public const string MenuUnfoldOutlined = "MenuUnfoldOutlined";
    public const string Search = "SearchOutlined";
}
